...
![image-20220402142531468](img(你真的了解final吗？)/image-20220402142531468.png)

![image-20220402142248572](img(你真的了解final吗？)/image-20220402142248572.png)
...
final局部变量由程序员进行显式初始化，如果final局部变量已经进行了初始化则后面就不能再次进行更改，如果final变量未进行初始化，可以进行赋值，**当且仅有一次**赋值，一旦赋值之后再次赋值就会出错。下面用具体的代码演示final局部变量的情况：
 ...
 ![image-20220402143020753](img(你真的了解final吗？)/image-20220402143020753.png)
...
然后FinalExample继承该父类，当重写test()方法时出现报错，如下图：
 ...
 ![final方法不能被重写](img(你真的了解final吗？)/final方法不能被重写.png)
...
父类会被final修饰，当子类继承该父类的时候，就会报错，如下图：
 ...
 ![final类不能继承.png](img(你真的了解final吗？)/final类不能继承.png)
...
我们来看一下一种执行时序图，如下：
 ...
 ![image-20220402155855198](img(你真的了解final吗？)/image-20220402155855198.png)
...
假设线程A写过程没有重排序，那么线程A和线程B有一种的可能执行时序为下图：
 ...
 ![image-20220402160542975](img(你真的了解final吗？)/image-20220402160542975.png)
...
针对上面的实例程序，线程线程A执行wirterOne方法，执行完后线程B执行writerTwo方法，然后线程C执行reader方法。下图就以这种执行时序出现的一种情况来讨论（耐心看完才有收获）。
 ...
 ![image-20220402161241738](img(你真的了解final吗？)/image-20220402161241738.png)
...
可能的执行时序如图所示：
 ...
 ![image-20220402162623340](img(你真的了解final吗？)/image-20220402162623340.png)
