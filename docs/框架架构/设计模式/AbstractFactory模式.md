...
在Abstract Factory模式中增加具体的工厂是非常容易的。这里说的“容易”指的是需要编写哪些类和需要实现哪些方法都非常清楚。无论要增加多少个具体工厂（或是要修改具体工厂的Bug)，都无需修改抽象工厂和Main部分。

## 难以增加新的零件

请试想一下要在Abstract Factory模式中增加新的零件时应当如何做。例如，我们要增加一个表示图像的Picture零件。这时，我们必须要对所有的具体工厂进行相应的修改才行。例如，我们必须要做以下修改。

- 在AbstractFactory中加入getPicture方法
- 新增PictureFactory类
