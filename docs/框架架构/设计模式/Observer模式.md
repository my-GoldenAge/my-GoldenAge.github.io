...
![观察者设计模式](image/observer.png)

## 问题

假如你有两种类型的对象：  `customer` 和 `shop` 。 顾客对某个特定品牌的产品非常感兴趣 （例如最新型号的 iPhone 手机）， 而该产品很快将会在商店里出售。

顾客可以每天来商店看看产品是否到货。 但如果商品尚未到货时， 绝大多数来到商店的顾客都会空手而归。

![访问商店或发送垃圾邮件](image/observer-comic-1-zh.png "前往商店和发送垃圾邮件")
...
观察者模式建议你为发布者类添加订阅机制， 让每个对象都能订阅或取消订阅发布者事件流。 不要害怕！ 这并不像听上去那么复杂。 实际上， 该机制包括 1） 一个用于存储订阅者对象引用的列表成员变量； 2） 几个用于添加或删除该列表中订阅者的公有方法。
 ...
 ![订阅机制](image/solution1-zh-16538878941083.png "订阅机制允许对象订阅事件通知。")
...
因此， 所有订阅者都必须实现同样的接口， 发布者仅通过该接口与订阅者交互。 接口中必须声明通知方法及其参数， 这样发布者在发出通知时还能传递一些上下文数据。
 ...
 ![通知方法](image/solution2-zh-16538879256105.png "发布者调用订阅者对象中的特定通知方法来通知订阅者。")

如果你的应用中有多个不同类型的发布者， 且希望订阅者可兼容所有发布者， 那么你甚至可以进一步让所有订阅者遵循同样的接口。 该接口仅需描述几个订阅方法即可。 这样订阅者就能在不与具体发布者类耦合的情况下通过接口观察发布者的状态。

## 真实世界类比

![杂志和报纸订阅](image/observer-comic-2-zh.png "杂志和报纸订阅。")
...
![观察者设计模式的结构](image/structure-16538879994188.png)
...
```java
package com.llh.observer.publisher;

import com.llh.observer.listeners.EventListener;
...
```java
package com.llh.observer.editor;

import com.llh.observer.publisher.EventManager;
...
```java
package com.llh.observer.listeners;
...
```java
package com.llh.observer;

import com.llh.observer.editor.Editor;
import com.llh.observer.listeners.EmailNotificationListener;
import com.llh.observer.listeners.LogOpenListener;
